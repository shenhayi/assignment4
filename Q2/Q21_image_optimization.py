...
def optimize_an_image(
    ...
    for i in tqdm(range(total_iter)):
        ...
        if args.sds_guidance:
            loss = sds.sds_loss(latents, embeddings["default"], embeddings["uncond"], guidance_scale=100, grad_scale=1)
        else:
            loss = sds.sds_loss(latents, embeddings["default"], None, guidance_scale=0, grad_scale=1)
